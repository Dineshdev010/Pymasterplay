...
export const categoryOrder = ["Beginner", "Intermediate", "Advanced", "Expert"] as const;

export const categoryTone: Record<string, { label: string; heading: string; badge: string; bg: string }> = {
  Beginner: {
...
export function getLessonHeadings(content: string) {
  return content
    .split("\n")
...
export type LessonClarityGuide = {
  summary: string;
  analogy: string;
  steps: string[];
  whereToUse: string[];
...
export type LessonUsagePlaybook = {
  whenToUse: string[];
  howToApply: string[];
...
export const keywordGuides: Record<string, KeywordGuide> = {
...
export function getKeywordExample(token: string): string | undefined {
  const t = token.trim();
...
export function withKeywordExample(token: string, guide: KeywordGuide): KeywordGuide {
  if (guide.example) return guide;
  const example = getKeywordExample(token);
  return example ? { ...guide, example } : guide;
}

export function getFallbackKeywordGuide(token: string): KeywordGuide | null {
...
export function getKeywordGuidesForLine(line: string): Array<{ token: string; guide: KeywordGuide }> {
...
export function getLessonClarityGuide(lessonId: string, title: string, language: LearnLanguage): LessonClarityGuide {
  if (language !== "english") {
...
export function getDataTypeUsageGuide(lessonId: string): DataTypeUsageItem[] {
...
export function getLessonNarrative(lessonId: string, title: string, language: LearnLanguage): LessonNarrative {
  if (language !== "english") {
    if (lessonId === "fundamentals") {
      if (language === "tamil") {
        return {
...
export function getLessonQuickExamples(title: string): QuickExample[] {
  const lower = title.toLowerCase();
...
export function getLessonUsagePlaybook(title: string): LessonUsagePlaybook {
  const lower = title.toLowerCase();
  ...
  return {
    whenToUse: ["When solving a specific algorithmic or data transformation problem", "When improving code readability", "When following best practices"],
    howToApply: ["Identify the inputs", "Apply the core method or syntax", "Validate the output"],
    starterPattern: `# Write your code here\n# Run it using the compiler\nprint("Success")`,
  };
}

export const pageTextTranslations = {
  english: {
