    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-card/95 backdrop-blur-md pb-[env(safe-area-inset-bottom)]">
      ...
      {mobileNavItems.map((item) =>
        <Link
          key={item.to}
          to={item.to}
          className={`flex w-full flex-col items-center gap-0.5 rounded-md px-1 py-1 text-[10px] transition-all duration-200 ${
        ...
        >
          ...
          <span className="text-[1.05rem] leading-none">{item.emoji}</span>
