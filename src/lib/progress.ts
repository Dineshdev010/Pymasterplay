...
export interface UserProgress {
  ...
  avatarUrl: string | null; // User's profile picture
}
...
export function recordActivity(progress: UserProgress): ActivityResult {
  ...
  updated.activityMap[today] = (updated.activityMap[today] || 0) + 1;
...
export function recoverStreak(progress: UserProgress): UserProgress | null {
  ...
  return {
    ...progress,
    wallet: progress.wallet - cost,
    streak: progress.previousStreak + 1,
    previousStreak: 0,
    streakBrokenDate: null,
  };
}
